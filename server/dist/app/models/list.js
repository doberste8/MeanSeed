...
function list() {
    ...
    this.get = function (res) {
        db.acquire(function (err, con) {
            ...
            con.query('select * from list where count = 1', function (err, result) {
                con.release();
                if (!err) {
                    res.send(result);
                }
    ...
    this.create = function (data, res) {
        db.acquire(function (err, con) {
            if (err)
                throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('INSERT INTO list set ?', data, function (err, result) {
                con.release();
                if (err)
                    res.send({
                        status: 1,
    ...
    this.update = function (data, res) {
        db.acquire(function (err, con) {
            if (err)
                throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('update list set ? where id like ?', [data, data.id], function (err, result) {
                con.release();
                if (err)
                    res.send({
                        status: 1,
    ...
    this.updateAll = function (data, res) {
        db.acquire(function (err, con) {
            if (err)
                throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('update list set ?', data, function (err, result) {
                con.release();
                if (err)
                    res.send({
                        status: 1,
